...
class BytecodeSegmentedNode extends BytecodeSegmentStructure {
  ...
  @override
  BigInt hash() {
    final hashInputs = <BigInt>[];
    for (final node in segments) {
...
BigInt computeCompiledClassHashInner(
  ...
  NestedIntList bytecodeSegmentLengths,
) {
  final bytecodeHash = createBytecodeSegmentStructure(
    bytecode: bytecode,
...
BytecodeSegmentStructure createBytecodeSegmentStructure({
  ...
  final revVisitedPcs =
      (visitedPcs ?? List<BigInt>.generate(bytecode.length, BigInt.from))
          .reversed
          .toList();

  final result = _createBytecodeSegmentStructureInner(
    bytecode: bytecode,
    bytecodeSegmentLengths: bytecodeSegmentLengths,
  ...
  assert(
    result.totalLen == BigInt.from(bytecode.length),
...
_BytecodeSegmentStructureResult _createBytecodeSegmentStructureInner({
  ...
  if (bytecodeSegmentLengths is int) {
    final segmentEnd = bytecodeOffset + BigInt.from(bytecodeSegmentLengths);
  ...
  if (bytecodeSegmentLengths is List) {
    final res = <BytecodeSegment>[];
    var totalLen = BigInt.from(0);

    for (final item in bytecodeSegmentLengths) {
      final visitedPcBefore = visitedPcs.isNotEmpty ? visitedPcs.last : null;

      final currentResult = _createBytecodeSegmentStructureInner(
        bytecode: bytecode,
      ...
      final visitedPcAfter = visitedPcs.isNotEmpty ? visitedPcs.last : null;
      final isUsed = visitedPcAfter != visitedPcBefore;
